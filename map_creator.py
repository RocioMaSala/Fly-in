...
class ZoneTypeError(Exception):
    def __init__(self, line_num: int, message: str = "Incorrect Zone Type") -> None:
        super().__init__(f"Line {line_num}, {message}")

class CapacityError(Exception):
    def __init__(self, line_num: int, message: str = "Invalid capacity") -> None:
        super().__init__(f"Line {line_num}, {message}")
...
def parse_zone(line: str, line_num:int) -> Zone:
    ...
    name = parts[1]
    if "-" in name:
        raise ValueError(f"Parsing Error: Zone name '{name}' cannot contain dashes, line {line_num}")
    
    try:
        coord_x = int(parts[2])
        coord_y = int(parts[3])
    except (IndexError, ValueError):
        raise ValueError(f"Parsing Error: Invalid or missing integer coordinates, line {line_num}")
    ...
    if "[" in line:
        try:
            ...
            metadata_dict = {k: v for k, v in (pair.split("=") for pair in metadata_pairs)}
            ...
            raise ValueError(f"Parsing Error: Invalid metadata syntax, line {line_num}")
    
    return Zone(name, coord_x, coord_y, start_zone, finish_zone, zone_type, color)
...
def parse_connection(line: str, line_num:int) -> Connection:
    ...
    try:
        ...
        raise ValueError(f"Parsing Error: Invalid connection format, expected 'zone1-zone2', line {line_num}")
    
    if zone_start == zone_finish:
        raise ValueError(f"Parsing Error: A zone cannot connect to itself, line {line_num}")    
    ...
    if "[" in line:
        try:
            ...
            raise ValueError(f"Parsing Error: Invalid connection metadata syntax, line {line_num}")
